...
mArenaMixin.defaultSettings.profile.drCategories = {
    Stun = true,
    Incapacitate = true,
    Disorient = true,
    Silence = true,
...
mArenaMixin.defaultSettings.profile.drDynamicIcons = false

local drCategories = mArenaMixin.drCategories
local drList
local drTime = 18.5
local severityColor = {
    [1] = { 0, 1, 0, 1 },
...
mArenaMixin.defaultSettings.profile.drIcons = {
    Stun = 132298,
    Incapacitate = 136071,
    Disorient = 136183,
    Silence = 458230,
...
function mArenaFrameMixin:FindDR(combatEvent, spellID)
    ...
    -- Set Border & Texture
    frame.Icon:SetTexture(self.parent.db.profile.drDynamicIcons and C_Spell.GetSpellTexture(spellID) or
    self.parent.db.profile.drIcons[category])
    frame.Border:SetVertexColor(unpack(severityColor[frame.severity]))

    if currentLayout == "BlizzArenaModern" then
        frame.Icon:AddMaskTexture(frame.Mask)
        frame.Border:SetScale(0.6)
...
function mArenaFrameMixin:UpdateDRPositions()
...
function mArenaFrameMixin:ResetDR()
    for i = 1, #drCategories do
