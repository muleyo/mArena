local function getLayoutTable()
    ...
    for k, _ in pairs(mArenaMixin.layouts) do
        t[k] = mArenaMixin.layouts[k].name and mArenaMixin.layouts[k].name or k
    end
...
function mArenaMixin:GetLayoutOptionsTable(layoutName)
    local optionsTable = {
        arenaFrames = {
            order = 1,
            name = "Arena Frames",
            ...
            args = {
                ...
                theme = {
                    ...
                    args = {
                        theme = {
                            order = 1,
                            name = "Only available for |cff00b4ffBlizz|r Arena Modern layout.",
                            type = "description",
                        },
                    }
                }
...
function mArenaMixin:UpdateFrameSettings(db, info, val)
    if ( val ) then
        db[info[#info]] = val
    end
...
function mArenaMixin:UpdateCastBarSettings(db, info, val)
    if ( val ) then
        db[info[#info]] = val
    end
...
function mArenaMixin:UpdateDRSettings(db, info, val)
    local categories = {
        "Stun",
        "Incapacitate",
        "Disorient",
...
function mArenaMixin:UpdateSpecIconSettings(db, info, val)
    if ( val ) then
        db[info[#info]] = val
    end
...
function mArenaMixin:UpdateTrinketSettings(db, info, val)
    if ( val ) then
        db[info[#info]] = val
    end
...
function mArenaMixin:UpdateRacialSettings(db, info, val)
    if ( val ) then
        db[info[#info]] = val
    end
