mArenaMixin = {}
mArenaFrameMixin = {}
mArenaCastingBarExtensionMixin = {}
mArenaMixin.layouts = {}

mArenaMixin.defaultSettings = {
    profile = {
        currentLayout = "BlizzArenaModern",
        classColors = true,
        showNames = true,
        statusText = {
            usePercentage = false,
...
local UnitFrameHealPredictionBars_Update = UnitFrameHealPredictionBars_Update_mArena
...
function mArenaMixin:OnEvent(event)
...
local function ChatCommand(input)
    if not input or input:trim() == "" then
        LibStub("AceConfigDialog-3.0"):Open("mArena")
    else
        LibStub("AceConfigCmd-3.0").HandleCommand("mArena", "mArena", "mArena", input)
    end
end

function mArenaMixin:Initialize()
    if (db) then return end

    self.db = LibStub("AceDB-3.0"):New("mArenaDB", self.defaultSettings, true)
    db = self.db
    ...
    LibStub("AceConfig-3.0"):RegisterOptionsTable("mArena", self.optionsTable)
    LibStub("AceConfigDialog-3.0"):AddToBlizOptions("mArena")
    LibStub("AceConfigDialog-3.0"):SetDefaultSize("mArena", 700, 620)
    LibStub("AceConsole-3.0"):RegisterChatCommand("mArena", ChatCommand)

    self:SetLayout(nil, db.profile.currentLayout)
end

function mArenaMixin:RefreshConfig()
    self:SetLayout(nil, db.profile.currentLayout)
end

function mArenaMixin:layoutReload(layout)
    ...
    frame.TitleText:SetText("mArena")
...
function mArenaMixin:SetLayout(_, layout)
    if (InCombatLockdown()) then return end

    layout = mArenaMixin.layouts[layout] and layout or "BlizzArena"
    ...
    LibStub("AceConfigRegistry-3.0"):NotifyChange("mArena")
...
function mArenaMixin:SetupDrag(frameToClick, frameToMove, settingsTable, updateMethod)
...
function mArenaMixin:SetMouseState(state)
    for i = 1, 3 do
        local frame = self["arena" .. i]
...
function mArenaFrameMixin:OnLoad()
    ...
    self:RegisterForClicks("AnyDown", "AnyUp") -- this is for WOW10, if mArena to support previous versions there would need to be a check to still catch only AnyUp
...
function mArenaFrameMixin:OnEvent(event, eventUnit, arg1, arg2 )
    ...
    if (eventUnit and eventUnit == unit) then
        ...
        elseif (event == "UNIT_HEALTH") then
            ...
            if (currHp ~= self.currHp) then
                ...
                UnitFrameHealPredictionBars_Update_mArena(self)
...
function mArenaFrameMixin:Initialize()
...
function mArenaFrameMixin:OnEnter()
...
function mArenaFrameMixin:OnLeave()
    UnitFrame_OnLeave(self)

    self:UpdateStatusTextVisible()
end

function mArenaFrameMixin:UpdateVisible()
...
function mArenaFrameMixin:UpdatePlayer(unitEvent)
    local unit = self.unit
...
function mArenaFrameMixin:SetMysteryPlayer()
...
function mArenaFrameMixin:UpdateAbsorb(unit)
...
function UnitFrameHealPredictionBars_Update_mArena(frame)
...
function mArenaFrameMixin:GetClassAndSpec()
    local _, instanceType = IsInInstance()
...
function mArenaFrameMixin:UpdateClassIcon()
    if (
...
function mArenaFrameMixin:UpdateTrinketSammers(unit, spellId)
...
function mArenaFrameMixin:ResetLayout()
    self.currentClassIconTexture = nil
    self.currentClassIconStartTime = 0
...
function mArenaFrameMixin:SetPowerType(powerType)
...
function mArenaFrameMixin:FindAura()
    local unit = self.unit
...
function mArenaFrameMixin:FindInterrupt(event, spellID)
...
function mArenaFrameMixin:SetLifeState()
    local unit = self.unit
...
function mArenaFrameMixin:SetStatusText(unit)
...
function mArenaMixin:Test()
    ...
    for i = 1, 3 do
        ...
        for n = 1, #self.drCategories do
            ...
            drFrame.Cooldown:SetSize(drFrame.Icon:GetSize())

            if (db.profile.currentLayout) == "BlizzArenaModern" then
                -- Set Border
                drFrame.Icon:AddMaskTexture(drFrame.Mask)
                drFrame.Border:SetScale(0.6)
                drFrame.Cooldown:SetSwipeTexture([[Interface\CharacterFrame\TempPortraitAlphaMask]])
            else
        ...
        if Masque ~= nil then
            mArena.MasqueGroup.CastBar:AddButton(frame.CastBar.Icon)
        end
...
mArenaCastingBarExtensionMixin.typeInfo = {
    applyingcrafting = { 
...
function mArenaCastingBarExtensionMixin:GetTypeInfo(barType)
